...
class GenericDataset(BaseDataset):
    ...
    def process_csv(self):
        ...
        df["img_path"] = df.apply(lambda x: os.path.join(self.data_root, "images", x["img_name"]), axis=1)
        return df
...
def build_random_benchmark_dataset(dataset_cfgs: list, total_num: int, output_dir: str, random_state: int = 42):
    print(f"📦 Building benchmark dataset with unified random sampling")
    print(f"🎯 Target: {total_num} images")
    
    all_data = []

    # Step 1: 合并所有 dataset 的 processed 数据
    ...
    merged_df = pd.concat(all_data).dropna(subset=["img_path"]).drop_duplicates(subset=["img_path"]).reset_index(drop=True)
    print(f"✅ Total unique images after merge: {len(merged_df)}")

    if len(merged_df) < total_num:
        raise ValueError(f"❌ Only {len(merged_df)} unique images available, cannot satisfy total_num={total_num}")

    # Step 2: 随机采样 total_num 张
    sampled_df = merged_df.sample(n=total_num, random_state=random_state).reset_index(drop=True)
...
def build_balanced_benchmark_dataset(dataset_cfgs, total_num, output_dir, random_state=42):
    print(f"Building balanced benchmark dataset...")
    all_data = []

    # 合并所有 dataset 的 processed 数据
    for cfg in dataset_cfgs:
        dataset = GenericDataset(
            dataset_name=cfg["name"],
            data_root=cfg["path"],
    ...
    merged_df = pd.concat(all_data).dropna(subset=["emotion_cat", "img_path"]).drop_duplicates(subset=["img_path"]).reset_index(drop=True)
    ...
    print(f"Sampling {num_per_class} per class × {num_classes} classes = {num_per_class * num_classes}")
    ...
    for label in emotion_labels:
        ...
        if len(class_subset) < num_per_class:
            print(f"⚠️ Class {label} only has {len(class_subset)} samples, not enough for {num_per_class}")
        sampled = class_subset.sample(n=min(num_per_class, len(class_subset)), random_state=random_state)
        balanced_samples.append(sampled)

    final_df = pd.concat(balanced_samples).drop_duplicates(subset=["img_path"]).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    with open(r"D:\dev\VLM-EQ\benchmark_datasets\example.yaml") as f:
